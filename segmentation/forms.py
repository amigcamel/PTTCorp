...
class segForm(forms.Form):
    box = forms.CharField(widget=forms.Textarea(attrs={'style':'width:500px'}), label=("Input any sentence or text"))
    upload_file  = forms.FileField(required=False, label=("Or upload a text file (optional)"), help_text="File size limit: %d KB" % FILE_SIZE_LIMIT)
