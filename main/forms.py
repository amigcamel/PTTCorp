...
class RegForm(RegistrationForm):
    # modified from here: /usr/local/lib/python2.7/dist-packages/registration/forms.py
    username = forms.RegexField(regex=r'^[a-zA-Z0-9]+$',
                                min_length=5,
                                max_length=30,
                                label=("Username"),
                                widget=forms.TextInput(attrs={'class': 'form-control'}),
                                error_messages={'invalid': ("Username may contain only letters and numbers")},
                                help_text="Username may contain at least 5 alphanumeric characters with only letters and numbers ")

    email = forms.EmailField(validators=[ajiEmailValidator], 
                             label=("E-mail"), 
