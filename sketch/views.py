...
def main(request):
    return render_to_response(
        'sketch.html', {}, context_instance=RequestContext(request))
...
def concordance(request):
    if 'query' in request.GET:
        ...
        if form.is_valid():
            ...
            if corpus == 'PTT':
                freq_by_year = [{
                    'year': str(year),
                    'count': float('%.4f' % (count * 100))
                } for year, count in cqp.freq_by_year.iteritems()]
    ...
    return render_to_response(
        'concordance.html', {'form': form},
        context_instance=RequestContext(request))


def source(request, board, mongoid, query):
    query = query.replace(' ', '').strip()
    res = connect('PTT', board).find({
    ...
    if urllib.urlopen(res['URL']).getcode() == 404:
        res['URL'] = u'deleted'
    res[u'content'] = re.sub(u'(' + query + u')', u'<span>\\1</span>',
                             res[u'content'])
    return render_to_response(
        'source.html', {'output': res},
        context_instance=RequestContext(request))
    ...
    if request.method == 'POST':
        ...
        form = CollocationForm(
            initial={
                ...
                'minimum_logDice': 0,
                'maxinum_number_of_items_displayed_in_a_grammatical_relation':
                30
            })
    return render_to_response(
        'collocation.html', {'form': form},
        context_instance=RequestContext(request))
