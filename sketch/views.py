...
def concordance(request):
    if 'query' in request.GET:
        ...
        if form.is_valid():
            ...
            if corpus == 'PTT':
                freq_by_year = [{'year': str(year), 'count': float('%.4f' % (
                    count * 100))} for year, count in cqp.freq_by_year.iteritems()]
            ...
            return render_to_response('concordance.html',
                                      {'output': output,
                                       'query': query,
                                       'window_size': window_size,
                                       'time_order': time_order,
                                       'freq_by_year': freq_by_year,
                                       'show_pos': repr(show_pos)},
                                      context_instance=RequestContext(request))
    else:
...
def source(request, board, mongoid, query):
    ...
    res = connect('PTT', board).find({'_id': ObjectId(mongoid)}, {
        '_id': 0, 'URL': 1, 'content': 1, 'post_time': 1, 'author': 1, 'title': 1})
    res = res.next()
    res[u'board'] = board
    if urllib.urlopen(res['URL']).getcode() == 404:
        res['URL'] = u'deleted'
    res[u'content'] = re.sub(
        u'(' + query + u')',
        u'<span>\\1</span>',
        res[u'content'])
    return render_to_response(
        'source.html', {
            'output': res}, context_instance=RequestContext(request))
...
def collocation(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            query = form.cleaned_data['query']
            res = connect('PTTcollo', 'collo').find(
                {'_id': query}, {'collos': 1, 'occ': 1})
            try:
                res = res.next()
                occ, output = res['occ'], res['collos']
                return render_to_response(
                    'collocation.html', {
                        'output': output, 'query': query, 'occ': occ}, context_instance=RequestContext(request))
            except BaseException:
                request.flash['notice'] = '*No result with "%s"' % query
                return HttpResponseRedirect(reverse('sketch_collocation'))
    else:
        form = CollocationForm(
            initial={
