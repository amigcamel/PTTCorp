...
from pttcorp_admin.views import super_user_auth #This is the decorator which only allows super to access certain pages
...
urlpatterns = patterns('',
    ...
    url('^download_res/(\w+)/$', segmentation.views.download, name="download_res"),
    ...
    url('^source/([a-zA-Z0-9_-]+)/(\w+)/(.+)/$', sketch.views.source, name='sketch_source'),
    ...
    url('^wordcloud/$', TemplateView.as_view(template_name='wordcloud.html'), name="wordcloud_main"),
    url('^wordcloud/(\w+)/$', wordcloud.views.genWordcloud, name='genWordcloud'),
    url('^pttcorp_admin/$', super_user_auth(TemplateView.as_view(template_name='admin_base.html')), name="pttcorp_admin"),
